    ...
    try {
      ...
      if (rememberMe) {
        try {
          ...
          console.log(
            `✓ Remember Me session created for user: ${user.email} (${deviceInfo.deviceName})`
          );
        } catch (sessionError) {
    ...
    try {
      ...
      if (!user) {
        return res.status(404).json({
          success: false,
          message:
            "We couldn't find an account with that email address. Please double-check your email or create a new account.",
          error:
            "We couldn't find an account with that email address. Please double-check your email or create a new account.",
        });
      }
