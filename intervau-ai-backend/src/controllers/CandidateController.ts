...
export class CandidateController {
  ...
  static async getDashboardStats(req: Request, res: Response) {
    try {
      ...
      let improvementLabel = 'Need more interview data';

      // Primary metric: compare current 30-day average against the previous 30-day average.
      if (currentPeriodItems.length > 0 && previousPeriodItems.length > 0) {
        improvementRate =
          previousPeriodAvg > 0
            ? Math.round(((currentPeriodAvg - previousPeriodAvg) / previousPeriodAvg) * 100)
            : 0;
        improvementLabel = 'Last 30 days';
      } else if (timeline.length >= 6) {
        // Fallback metric for newer users: compare first 3 sessions vs latest 3 sessions.
        const sortedTimeline = [...timeline].sort(
          (a, b) => a.occurredAt.getTime() - b.occurredAt.getTime()
        );
        const firstThree = sortedTimeline.slice(0, 3);
        const latestThree = sortedTimeline.slice(-3);
        const firstThreeAvg = calculateAvg(firstThree);
        const latestThreeAvg = calculateAvg(latestThree);

        improvementRate =
          firstThreeAvg > 0
            ? Math.round(((latestThreeAvg - firstThreeAvg) / firstThreeAvg) * 100)
            : 0;
        improvementLabel = 'Recent sessions';
      }
  ...
  static async deleteResumeAnalysis(req: Request, res: Response) {
    try {
      ...
      const deleted = await ResumeAnalysis.findOneAndDelete({ userId });
