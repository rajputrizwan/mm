...
const transporter = nodemailer.createTransport({
  ...
  rateDelta: 1000,     // define millisecond rate window
  rateLimit: 5,        // max messages per rate window
...
/**
 * Send contact form notification email (with retry)
 */
...
You can reply to this inquiry at: ${email}
    `.trim();
      ...
      console.log(`✓ Contact notification email sent (attempt ${attempt}):`, info.messageId);
      return; // success — exit retry loop
    } catch (error) {
      lastError = error;
      console.warn(`⚠️ Contact notification attempt ${attempt}/${maxRetries} failed:`, (error as Error).message);
  ...
  console.error('❌ Failed to send contact notification email after retries:', lastError);
  throw lastError;
};
