...
/**
 * Validation rules for resume role-context payload.
 */
export const resumeRoleContextValidation = [
  body('targetJobPosition')
    .optional({ values: 'falsy' })
    .isString()
    .withMessage('Target job title must be a string')
    .trim()
    .isLength({ max: 200 })
    .withMessage('Target job title must not exceed 200 characters'),
  body('targetJobDescription')
    .optional({ values: 'falsy' })
    .isString()
    .withMessage('Target job description must be a string')
    .trim()
    .isLength({ max: 4000 })
    .withMessage('Target job description must not exceed 4000 characters'),
];

/**
 * Validation rules for updating a candidate resume URL.
 */
export const updateResumeValidation = [
  body('resumeUrl')
    ...
    .withMessage('Resume URL must be between 1 and 2048 characters')
