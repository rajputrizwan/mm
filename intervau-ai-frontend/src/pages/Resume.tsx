...
export default function Resume() {
  ...
  const [lastRejectionCode, setLastRejectionCode] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    const loadAnalysis = async () => {
      ...
      if (response.success && response.data) {
        const normalized = normalizeResumeAnalysis(response.data);
        if (normalized) {
          setAnalysisResult(normalized);
          setTargetJobPosition(normalized.targetJobPosition || "");
          setTargetJobDescription(normalized.targetJobDescription || "");
          localStorage.setItem("resumeAnalysis", JSON.stringify(normalized));
        }
      ...
      // Network / CORS failure — request wrapper returns statusCode undefined.
    ...
    async (acceptedFiles: File[]) => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          const skillCount = normalized.extractedSkills.length;
          addNotification(
  ...
  const handleRegenerateInsights = async () => {
    ...
    try {
      ...
      if (!normalized) {
        ...
        toast.error("Received invalid analysis data. Please try again.");
        return;
      }
  ...
  useEffect(() => {
    if (fileRejections.length > 0) {
      ...
      const rejectionCode = rejection.errors[0]?.code;
      if (!rejectionCode || rejectionCode === lastRejectionCode) {
        return;
      }

      setLastRejectionCode(rejectionCode);

      if (rejectionCode === "file-too-large") {
        setError("File size must be less than 10 MB.");
        toast.error("File size must be less than 10 MB.");
      } else if (rejectionCode === "file-invalid-type") {
        setError("Only PDF, DOC, and DOCX files are supported.");
        toast.error("Only PDF, DOC, and DOCX files are supported.");
      }
    }
  }, [fileRejections, lastRejectionCode]);
  ...
  const suggestedQuestions = analysisResult?.suggestedQuestions ?? [];
  const analyzedAtLabel = formatAnalysisDate(analysisResult?.analyzedAt);
  const showStaleAnalysisWarning = isAnalysisStale(analysisResult?.analyzedAt);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div
            {...getRootProps()}
            className={`bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-12 border-2 border-dashed transition-all cursor-pointer ${
              isDragActive
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-4 py-3 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
          ...
          <div className="mt-12 grid md:grid-cols-3 gap-6">
            <div className="text-center">
              <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-200 dark:border-gray-700 hover:border-blue-500/50 transition-all">
            ...
            <div className="text-center">
              <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-200 dark:border-gray-700 hover:border-green-500/50 transition-all">
            ...
            <div className="text-center">
              <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-200 dark:border-gray-700 hover:border-purple-500/50 transition-all">
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="inline-flex items-center space-x-2 bg-emerald-50 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300 px-3 py-1 rounded-full text-sm font-medium mb-3 border border-emerald-200 dark:border-emerald-500/30">
            ...
            <p className="text-sm md:text-base text-gray-600 dark:text-gray-400 mt-2">
              Review extracted skills, identify gaps, and tailor insights to
              your target role.
            </p>
          </div>
          <button
            onClick={handleNewUpload}
            className="inline-flex items-center justify-center px-6 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-xl font-medium hover:shadow-lg hover:border-blue-400/60 transition-all"
          >
            Upload new resume
          </button>
        </div>

        {showStaleAnalysisWarning && (
          <div className="mb-6 flex items-start gap-3 rounded-2xl border border-amber-300/80 dark:border-amber-700/70 bg-amber-50 dark:bg-amber-900/20 px-4 py-3 text-amber-800 dark:text-amber-200">
            <AlertCircle className="w-5 h-5 mt-0.5 flex-shrink-0" />
            <div>
              <p className="font-semibold text-sm">
                Analysis might be outdated
              </p>
              <p className="text-sm opacity-90">
                This resume analysis is older than 30 days. Regenerating
                insights can improve relevance for current roles.
              </p>
            </div>
          </div>
        )}

        <div className="mb-8 bg-white/95 dark:bg-gray-800/95 backdrop-blur rounded-3xl shadow-xl p-6 md:p-8 border border-gray-200/80 dark:border-gray-700">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between mb-6">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
                Regenerate insights
              </h2>
              <p className="text-sm md:text-base text-gray-600 dark:text-gray-400 max-w-3xl">
                Update target role details and regenerate match score, skill
                gaps, and suggested questions with role-specific context.
              </p>
            </div>
            <div className="inline-flex items-center gap-2 px-3 py-2 rounded-full bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-xs text-gray-600 dark:text-gray-300">
              <span className="h-2 w-2 rounded-full bg-blue-500" />
              Last updated {analyzedAtLabel}
            </div>
          </div>
          ...
          <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-2xl font-semibold bg-gradient-to-r from-blue-600 to-cyan-600 text-white hover:shadow-xl hover:shadow-blue-500/20 disabled:opacity-70 disabled:cursor-not-allowed transition-all"
            >
              {regenerating ? (
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Tip: Add a concise job description for more accurate match
              scoring.
            </p>
          </div>

          {error && (
            <div className="mt-4 flex items-start gap-2 rounded-xl border border-red-300/80 dark:border-red-800 bg-red-50 dark:bg-red-900/20 px-4 py-3 text-sm text-red-700 dark:text-red-300">
        ...
        <div className="grid lg:grid-cols-3 gap-6 mb-8">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6 border border-gray-200 dark:border-gray-700">
            ...
            <p className="text-xl font-semibold text-gray-900 dark:text-white break-words">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6 border border-gray-200 dark:border-gray-700">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              Across {new Set(extractedSkills.map((s) => s.category)).size}{" "}
              categories
            </p>
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6 border border-gray-200 dark:border-gray-700">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {extractedSkills.map((skill) => (
                  <div
                    key={skill.name}
                    className="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50/80 dark:bg-gray-700/40"
                  >
              ...
              {skillGaps.length > 0 ? (
                <div className="space-y-4">
                  {skillGaps.map((gap, idx) => (
                    <div
                      key={idx}
                      className="p-4 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-700/30 rounded-xl"
                    >
                      <div className="flex items-start justify-between mb-2 gap-3">
                        <h3 className="font-semibold text-gray-900 dark:text-white">
                          {gap.skill}
                        </h3>
                        <span className="text-xs px-2 py-1 bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 rounded-full border border-amber-300/70 dark:border-amber-700/50 whitespace-nowrap">
                          {gap.importance}
                        </span>
                      </div>
                ...
                <div className="rounded-xl border border-emerald-200 dark:border-emerald-800/40 bg-emerald-50/70 dark:bg-emerald-900/10 p-4 text-sm text-emerald-700 dark:text-emerald-300">
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-blue-600 via-blue-600 to-cyan-600 rounded-2xl shadow-md p-6 text-white border border-blue-500/40">
              <Sparkles className="w-8 h-8 mb-3" />
              <h2 className="text-xl font-bold mb-2">Interview questions</h2>
              <p className="text-blue-100 text-sm mb-5">
                Practice these tailored prompts to strengthen weak areas.
              </p>
                ...
                {suggestedQuestions.map((q, idx) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5 gap-3">
                      ...
                      <span className="text-xs px-2 py-0.5 bg-white/30 rounded-full capitalize whitespace-nowrap">
                        {q.difficulty}
                      </span>
                    </div>
