  ...
  const handleSaveInterview = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        toast.error(response.error || "Unable to create the interview.");
      }
    } catch (error: any) {
      console.error("Error saving interview:", error);
      toast.error("Unable to save the interview. Please try again.");
    } finally {
      setSaving(false);
    }
  };
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {DIFFICULTY_LEVELS.map((level) => (
                        <button
                          ...
                          className={`p-3 rounded-lg border-2 transition-all text-center ${
                            formData.aiSettings.difficultyLevel === level.value
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 ${
                          formData.aiSettings.autoScore
                            ? "bg-green-500"
                    ...
                    <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                      <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          Provide detailed feedback to candidates after
                          interview
                        </p>
                      </div>
                      <button
                        type="button"
                        onClick={() =>
                          handleAiSettingToggle("enableAiFeedback")
                        }
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 ${
                          formData.aiSettings.enableAiFeedback
                            ? "bg-green-500"
                            : "bg-gray-300 dark:bg-gray-600"
                        }`}
                      >
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center mb-2">
                          <span
                            className={`${getQuestionTypeColor(question.type).bg} ${getQuestionTypeColor(question.type).text} text-xs font-semibold px-2.5 py-1 rounded-full mr-2 capitalize border ${getQuestionTypeColor(question.type).border}`}
                          >
