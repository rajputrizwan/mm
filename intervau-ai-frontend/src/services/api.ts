...
export function setAuthToken(token: string, rememberMe = false): void {
  if (rememberMe) {
    ...
    sessionStorage.removeItem("authToken"); // clear any stale session token
  } else {
    sessionStorage.setItem("authToken", token);
    localStorage.removeItem("authToken"); // don't persist across restarts
  }
}
...
export const api = {
  ...
  /** Completed mock interview history list (lightweight payload). Uses GET /history. */
  getCompletedMockInterviewHistory: (params?: {
    ...
    }>("/interviews/mock-interviews/history", {
      params: {
