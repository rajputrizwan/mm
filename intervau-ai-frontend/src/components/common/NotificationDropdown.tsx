...
export default function NotificationDropdown({
  ...
  return (
    <div className="absolute right-0 top-full mt-2 w-96 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
      {/* Header */}
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {panelNotifications.map((notification) => {
              ...
              return (
                <div
                  key={notification.id}
                  className={`flex items-start space-x-3 p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer ${
                    !notification.read
                ...
                >
                  <div
                    className={`flex-shrink-0 mt-0.5 ${iconColors[notification.type]}`}
                  >
      ...
      {panelNotifications.length > 0 && (
        <div className="px-4 py-3 bg-gray-50 dark:bg-gray-700/50 border-t border-gray-200 dark:border-gray-600">
          <p className="text-xs text-center text-gray-500 dark:text-gray-400">
            {panelNotifications.length} notification
            {panelNotifications.length !== 1 ? "s" : ""}
          </p>
        </div>
      )}
    </div>
