...
export default function QuestionCard({
  ...
  if (variant === 'compact') {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 hover:border-blue-300 dark:hover:border-blue-500 transition-colors">
        <div className="flex items-start justify-between mb-2">
          {number && total && (
            <span className="text-xs font-semibold text-gray-500 dark:text-gray-400">Q{number}/{total}</span>
          )}
        ...
        <p className="text-sm text-gray-700 dark:text-gray-300 line-clamp-2">{question}</p>
        {duration && (
          <div className="flex items-center space-x-1 text-gray-500 dark:text-gray-400 text-xs mt-2">
            <Clock className="w-3 h-3" />
            <span>{duration}</span>
          </div>
        )}
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 border border-gray-100 dark:border-gray-700">
      <div className="flex items-start space-x-3">
        <div className="bg-gradient-to-br from-blue-500 to-cyan-500 dark:from-blue-600 dark:to-cyan-600 p-2 rounded-lg flex-shrink-0">
          <MessageSquare className="w-5 h-5 text-white" />
        </div>
        <div className="flex-1">
          <div className="flex items-center justify-between mb-3">
